revoke select on public.server_operation_logs from authenticated;

drop policy if exists "Authenticated users can read server operation logs"
...
as $$
  ...
  owned as (
    select
      ...
      (array_agg(coalesce(nullif(series_title, ''), title) order by volume_number nulls last, created_at desc))[1] as title,
      (array_agg(thumbnail_url order by (thumbnail_url is null), volume_number nulls last, created_at desc))[1] as cover_url,
  ...
  ranked as (
    select
      ...
      coalesce(wanted.average_score, 0) as average_score,
      coalesce(wanted.top_score, 0) as top_score,
      ...
      (
        ...
        + coalesce(wanted.average_score, 0)::numeric * 0.03
      ) as popularity_score
    from wanted
